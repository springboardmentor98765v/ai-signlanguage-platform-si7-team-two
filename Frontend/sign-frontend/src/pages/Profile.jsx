...
export default function Practice() {
  ...
  const [lessonListWarning, setLessonListWarning] = useState("");
  ...
  useEffect(() => {
    async function loadLessons() {
      try {
        ...
        setLessonList(data);
        setLessonListWarning("");
      } catch (err) {
        console.error("Failed to load lessons:", err);
        setLessonList([]);
        // Milestone 3, Day 7: tell the learner why the letter picker is
        // limited, instead of silently falling back with no explanation.
        setLessonListWarning(
          "Couldn't load the full lesson list — showing the current letter only.",
        );
      }
    }
    ...
    try {
      ...
      try {
        ...
        if (user && lesson) {
          ...
          // Milestone 3, Day 7: this used to only warn in the console —
          // the learner had no idea their practice wasn't being tracked.
          setSessionWarning(
            "Heads up: we couldn't start a tracked session, so this practice won't be saved to your progress.",
          );
        }
      } catch (err) {
    ...
    <div>
      ...
      <div className="practice-grid">
        <div className="practice-panel">
          ...
          {sessionWarning && <p className="camera-error" role="alert">{sessionWarning}</p>}
